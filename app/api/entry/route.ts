...
const BOOT_NAMES = GOLDEN_BOOT.map((p) => p.name);
const BALL_NAMES = GOLDEN_BALL.map((p) => p.name);
const GLOVE_NAMES = GOLDEN_GLOVE.map((p) => p.name);
...
function isValid(b: Partial<WorldCupEntry>): b is WorldCupEntry {
  return (
    ...
    typeof b.team === "string" && typeof b.teamCode === "string" &&
    TEAMS.some((t) => t.code === b.teamCode && t.name === b.team) &&
    typeof b.goldenBoot === "string" && BOOT_NAMES.includes(b.goldenBoot) &&
    typeof b.goldenBall === "string" && BALL_NAMES.includes(b.goldenBall) &&
    typeof b.goldenGlove === "string" && GLOVE_NAMES.includes(b.goldenGlove)
  );
}
...
export async function POST(req: Request) {
  let body: Partial<WorldCupEntry>;
  try {
    body = await req.json();
  } catch {
