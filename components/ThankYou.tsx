"use client";

import { useRef } from "react";
import { gsap } from "gsap";
import { useGSAP } from "@gsap/react";
import ElbritLogo from "./ElbritLogo";
import TeamFlag from "./TeamFlag";
import { fireConfetti } from "@/lib/confetti";
import { WorldCupEntry } from "@/lib/types";

interface ThankYouProps {
  data: WorldCupEntry;
}

function firstName(full: string) {
  const parts = full.split(" ").filter((p) => !/^dr\.?$/i.test(p));
  return parts[0] || full.split(" ")[0] || "there";
}

export default function ThankYou({ data }: ThankYouProps) {
  const root = useRef<HTMLDivElement>(null);

  useGSAP(
    () => {
      gsap.from(".ty-ring-outer", { scale: 0, duration: 0.75, ease: "elastic.out(1,.45)", delay: 0.05 });
      gsap.from(".ty-reveal", {
        y: 18,
        opacity: 0,
        duration: 0.42,
        stagger: 0.13,
        delay: 0.5,
        ease: "power2.out",
      });

      fireConfetti();
    },
    { scope: root }
  );

  return (
    <div className="ty-wrap" ref={root}>
      <div className="logo-row sm ty-top">
        <ElbritLogo height={28} />
      </div>

      <div className="ty-ring-outer">
        <div className="tyr r1" />
        <div className="tyr r2" />
        <div className="tyr r3" />
        <div className="ty-inner">
          <span className="ty-flag">
            <TeamFlag code={data.teamCode} />
          </span>
        </div>
      </div>

      <p className="ty1 ty-reveal">Predictions locked,</p>
      <p className="ty2 ty-reveal">{firstName(data.name)}</p>
      <p className="ty3 ty-reveal">
        Get <b>all 4 right</b> and you&rsquo;re in line for the amazing prizes,
        announced by Elbrit after the Final. Good luck!
      </p>

      <div className="ty-card ty-reveal">
        <div className="tc-row">
          <span className="tc-lb">🏆 Champion</span>
          <span className="tc-vl">{data.team}</span>
        </div>
        <div className="tc-row">
          <span className="tc-lb">👟 Golden Boot</span>
          <span className="tc-vl">{data.goldenBoot}</span>
        </div>
        <div className="tc-row">
          <span className="tc-lb">⚽ Golden Ball</span>
          <span className="tc-vl">{data.goldenBall}</span>
        </div>
        <div className="tc-row">
          <span className="tc-lb">🧤 Golden Glove</span>
          <span className="tc-vl">{data.goldenGlove}</span>
        </div>
      </div>

      <p className="ty-footer ty-reveal">Winners announced by Elbrit after the Final · Entry confirmed</p>
    </div>
  );
}
