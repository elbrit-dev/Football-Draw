...
interface WizardFormProps {
  picks: Predictions;
  onSuccess: (data: WorldCupEntry) => void;
...
export default function WizardForm({ picks, onSuccess, onBack }: WizardFormProps) {
  const { team, boot, ball, glove } = picks;
  const [step, setStep] = useState(0);
  const [data, setData] = useState<WorldCupEntry>(EMPTY);
  ...
  async function submit() {
    ...
    setSubmitting(true);
    setServerError("");
    const payload: WorldCupEntry = {
      ...data,
      team: team.name,
      teamCode: team.code,
      goldenBoot: boot.name,
      goldenBall: ball.name,
      goldenGlove: glove.name,
    };
    try {
    ...
    <div className="form-wrap">
      ...
      <div className="team-badge">
        ...
        <div className="ntext">
          ...
          {team.name} · {boot.name} · {ball.name} · {glove.name}
        </div>
      </div>
      ...
      <div className="wiz-nav">
        ...
        {!isLast ? (
          ...
          <button className="btn-primary on grow" onClick={submit} type="button" disabled={submitting}>
            {submitting ? "Submitting…" : "Lock my prediction →"}
          </button>
        )}
      </div>
    </div>
